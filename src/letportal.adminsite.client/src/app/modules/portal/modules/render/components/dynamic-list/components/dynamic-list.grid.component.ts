...
export class DynamicListGridComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    private constructGrid() {
        ...
        this.defaultSortDirection = splittedDefaultSort[1] as SortDirection
        if(colIndex > 0){
    ...
    onCommandClick(commandClicked: CommandClicked) {
    ...
    isDisplayedAsHtml(currentColumn: ExtendedColDef) {
        return currentColumn.displayFormatAsHtml
    }

    isBooleanAsHtml(currentColumn: ExtendedColDef) {
    ...
    isSelectAsHtml(currentColumn: ExtendedColDef) {
        return currentColumn.searchOptions.fieldValueType === FieldValueType.Select
    }

    isTextAsHtml(currentColumn: ExtendedColDef) {
        return !this.isBooleanAsHtml(currentColumn) && !this.isSelectAsHtml(currentColumn)
    }

    getBooleanValue(renderingData: any, currentColumn: ExtendedColDef) {
    ...
    renderInnerHtml(renderingData: any, currentColumn: ExtendedColDef) {
        if (currentColumn.name === 'id' || currentColumn.name === '_id') {
            const checkData = renderingData.id
            if (!checkData) {
                return renderingData._id

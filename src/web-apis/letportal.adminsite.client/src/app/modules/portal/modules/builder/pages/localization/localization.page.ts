...
export class LocalizationPage implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!ObjectUtils.isNotNull(this.localization)) {
      ...
      this.languageKeys = this.localization.localizationContents.map(a => ({
        key: a.key,
        value: a.text
      } as LanguageKey))
    }
  ...
  saveChanges() {
    if (this.formGroup.valid || this.isEditMode) {
      const combineLocalization: Localization = {
        ...
        appId: this.appId,
        localizationContents: this.languageKeys.map(a => ({
          key: a.key,
          text: a.value
      ...
      if (this.isEditMode) {
        this.localizationClient.delete(this.localization.id).pipe(
          switchMap(() => this.localizationClient.create(combineLocalization))
        ).subscribe(
          () => {
      ...
      tap(
        allKeys => {
          if (this.isEditMode) {
            ...
            const removedItems: LanguageKey[] = []
  ...
  onFileChange($event: Event) {
    ...
    const latestFile: File = input.files[input.files.length - 1]
    if (ObjectUtils.isNotNull(latestFile)) {
      if (latestFile.name.indexOf('.json') < 0) {
        ...
        reader.onload = () => {
          try {
            this.localization = JSON.parse(reader.result as string)
            ...
            window.alert('Something went wrong: ' + (ex as Error).message)
          }
        }
